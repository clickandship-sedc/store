...
function User() {
  ...
  useEffect(() => {
    setCompletedOrders(data?.userOrders?.filter(order => order.status === 'Delivered'))
    setPendingOrders(data?.userOrders?.filter(order => order.status === 'OnTheWay' || order.status === 'Purchased'))
  }, [data?.userOrders])
  ...
  return (
    <>
      {isUserPending ? (
            ...
            <div className="adressDiv">
              ...
              <ul>
                ...
                <li>
                  ...
                  <p>Postal Code: {data.postalCode || 'Unknown'}</p>
                </li>
                <li>
                  <svg viewBox="0 0 24 24">
                    <path
                  ...
                  <p>Street: {data.address || 'Unknown'}</p>
                </li>
              </ul>
            </div>
          </div>
        ...
        {data.userOrders?.length || orderItems?.length ? (
            ...
            {completedOrders?.length > 0 &&
              <div className="completedOrders orderCo">
                ...
                <CSSTransition
                  ...
                  <ul className={`orderUL ${completedOrders.length === 1 && 'single'}`} ref={completedref}>
        ...
        {data.userRatings?.length ? (
          <div className="reviews">
            <ul className="reviews-ul">
              {data.userRatings.map((e) => (
                <UserReviews key={e.id} review={e} userImage={data.image} userName={`${data.firstName} ${data.lastName}`} userId={data.id} />
              ))}
            </ul>
          </div>
        ) : (
